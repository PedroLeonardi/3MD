...
class AuthController {
    ...
    static async login(req, res) {
        try {
            ...
            // Validação básica de formato de login
            const loginRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!loginRegex.test(login)) {
                return res.status(400).json({
                    sucesso: false,
                    erro: 'login inválido',
                    mensagem: 'Formato de login inválido'
                });
            }

            // Verificar credenciais
            const usuario = await UsuarioModel.verificarCredenciais(login.trim(), senha);
    ...
    static async registrar(req, res) {
        try {
            ...
            const usuarioExistente = await UsuarioModel.buscarPorEmail(login);
            if (usuarioExistente) {
    ...
    static async criarUsuario(req, res) {
        try {
            ...
            // Verificar se o login já existe
            const usuarioExistente = await UsuarioModel.buscarPorlogin(login);
            if (usuarioExistente) {
                return res.status(409).json({
                    sucesso: false,
                    erro: 'login já cadastrado',
    ...
    static async atualizarUsuario(req, res) {
        try {
            ...
            if (login !== undefined) {
                ...
                const usuarioComlogin = await UsuarioModel.buscarPorlogin(login);
                if (usuarioComlogin && usuarioComlogin.id !== parseInt(id)) {
                    return res.status(409).json({
                        sucesso: false,
                        erro: 'login já cadastrado',
                        mensagem: 'Este login já está sendo usado por outro usuário'
                ...
                dadosAtualizacao.login = login.trim().toLowerCase();
            }
